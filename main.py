...
def main():
    ...
    ips = generate_random_ips(ranges, COUNT_PER_RANGE)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        tasks = [executor.submit(test_ip, ip) for ip in ips]
        ...
        for task in as_completed(tasks):
            ...
            for item in results:
                ...
                if MIN_LATENCY <= latency <= MAX_LATENCY:
                    ...
                    print(f"[GOOD] {item['ip']}:{item['port']} ping={latency}ms")

    good_results.sort(key=lambda x: x["latency"])

    if good_results:
        save_results(good_results)
        print(f"Total good results: {len(good_results)}")
    else:
